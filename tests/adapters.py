...
def run_linear(
    ...
    """
    lnn = Linear(d_in, d_out, dtype=torch.float32)
    lnn.load_state_dict({"weight": weights})
    return lnn(in_features)
...
def run_embedding(
    ...
    """
    emb = Embedding(vocab_size, d_model, dtype=torch.float32)
...
def run_rmsnorm(
    ...
    """
    rmsnorm = RMSNorm(d_model, eps, dtype=torch.float32)
